...
class WebDriverModuleTest(unittest.TestCase):
    ...
    WEBDRIVERS = ['Android', 'Chrome', 'Firefox', 'Ie', 'Opera', 'PhantomJS', 'Remote', 'Safari']
    ...
    def test_module_exports_all_webdrivers(self):
        for driver_name in self.WEBDRIVERS:
            self.assertTrue(hasattr(self.module, driver_name), '{} was not found in the module!'.format(driver_name))

    def test_exported_webdrivers_are_classes(self):
        for driver_name in self.WEBDRIVERS:
            klass = getattr(self.module, driver_name)
            self.assertIsInstance(klass, type, '{} is not a class!'.format(driver_name))
    ...
    def test_exported_webdrivers_are_subclasses_of_webdriver_and_mixin(self):
        for driver_name in self.WEBDRIVERS:
            ...
            self.assertTrue(
                issubclass(pytractor_driver, selenium_driver),
                'pytractor.webdriver.{0} is not a subclass of ' 'selenium.webdriver.{0}'.format(driver_name),
            )
            self.assertTrue(
                issubclass(pytractor_driver, WebDriverMixin),
                'pytractor.webdriver.{} is not a subclass of ' 'pytractor.mixins.WebDriverMixin'.format(driver_name),
            )
